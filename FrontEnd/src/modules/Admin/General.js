...
@inject('repairStore', 'generalStore', 'userStore', 'adminGeneralStore')
@observer
class General extends Component {
    constructor(props) {
        ...
        this.store = {
            ...
            adminGeneralStore: props.adminGeneralStore
        };
    }
    ...
    componentDidMount() {
        ...
        this.store.adminGeneralStore.repairListPage = 1;
        this.store.adminGeneralStore.getRepair();
    }
    ...
    search=()=>{
        ...
        // console.log(this.props.form.getFieldValue('star_time').format('X'));
        console.log(data);
        this.store.adminGeneralStore.resetRepairList();
        this.store.adminGeneralStore.getRepair(data);
    };
    ...
    render() {
        ...
        return (
            <div id="admin-general">
                <ListView
                    ref="lv"
                    dataSource={this.store.adminGeneralStore.repairListDataSource}
                    renderRow={row}
                    renderFooter={() => (<div style={{padding: 30, textAlign: 'center'}}>
                        {this.store.adminGeneralStore.hasMore ? this.store.adminGeneralStore.isLoading ? '正在加载...' : '上拉加载' : '没有数据了'}
                    </div>)}
                    renderHeader={header}
                    pageSize={10}
                    ...
                    onEndReached={() => this.store.adminGeneralStore.getRepair()}
                    scrollerOptions={{scrollbars: true}}
                    refreshControl={<RefreshControl
                        refreshing={this.store.adminGeneralStore.refreshing}
                        onRefresh={() =>{this.store.adminGeneralStore.onRefresh();this.props.form.resetFields()} }
                    />}
                />
            </div>
        );
